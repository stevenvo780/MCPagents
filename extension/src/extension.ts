...
- Formatea la respuesta en Markdown`;
  ...
  public async callMCPServer(systemPrompt: string, prompt: string, temperature: number = 0.7): Promise<any> {
    try {
      const response = await fetch(SERVER_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const mcpAutonomousAsk = {
    invoke: async (options: any, token: vscode.CancellationToken) => {
      try {
        const { question, prompt, system, temperature, maxTokens, context: userContext } = options.input;
        const response = await languageModelTools.callMCPServer(
          system || 'Eres un asistente experto en programación.',
          question || prompt,
          temperature || 0.3
        );
        return new vscode.LanguageModelToolResult([
  ...
  const mcpAutonomousHealth = {
    invoke: async (options: any, token: vscode.CancellationToken) => {
      try {
        ...
        return new vscode.LanguageModelToolResult([
          new vscode.LanguageModelTextPart(JSON.stringify({ status: 'error', message: (error as Error).message }))
        ]);
      }
    }
  };
...
\`\`\``;
  ...
  context.subscriptions.push(
    vscode.lm.registerTool('mcp_autonomous_ask', mcpAutonomousAsk),
    vscode.lm.registerTool('mcp_autonomous_health', mcpAutonomousHealth),
    vscode.lm.registerTool('mcp_autonomous_analyze_code', mcpAutonomousAnalyzeCode)
  );
  ...
  const mcpAskCmd = vscode.commands.registerCommand('autonomousMcpHelper.mcpAsk', async () => {
    ...
    if (question) {
      try {
        const response = await languageModelTools.callMCPServer(
          'Eres un experto en desarrollo de software. Responde de manera práctica y con ejemplos.',
          question,
          0.3
        );
